...
void process_requests(http::request<http::string_body> &req,
    ...
    if (req.method() == http::verb::get) {  // GET /key HTTP/1.1:
        ...
        double space_used = cache->space_used();
        ...
        if (!std::isnan(space_used)) {
            ...
            //res.insert("Hitrate", std::to_string(hit_rate));
            res.http::basic_fields<std::allocator<char >>::insert("X-Clacks-Overhead", "GNU Terry Pratchett");
        } else {
            res.result(500);  // 500 Internal Server Error
        }
...
int main(int argc, char *argv[]) {
    ...
    auto usage = [&, argv](int status) {
        std::cout << "Usage: " << basename(argv[0]) << std::endl
                  << "\t-m [16777216]  Cache's capacity in bytes." << std::endl
    ...
    while ((option = getopt(argc, argv, "m:s:p:t:h")) != -1) {
        switch (option) {
            ...
            case 'p':
                ...
                break;
            case 't':
                threadcount = std::stoi(optarg, nullptr, 10);
                break;
            case 'h':
                usage(EXIT_SUCCESS);
                break;
    ...
    try {
        ...
        for (;;) {
            ...
            for(auto i = 0; i < threadcount; i++){
                threads[i] = std::thread(std::bind(&handle_sessions, std::move(sock)));
            }
            ioc.run();
