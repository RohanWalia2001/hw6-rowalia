...
public:
    ...
    http::response<http::dynamic_body> send(const http::verb &method,
        ...
        try {
            // Tell the tcp stream to expire after 30 seconds
            stream.expires_after(std::chrono::seconds(30));

            // Make the connection on the IP address we get from a lookup
            stream.connect(results, ec);
            if (ec) {
                std::cerr << "Impl::send(): stream.connect: " << ec.message()
                        << std::endl;
            }
            ...
            if (ec) {
                std::cerr << "Impl::send(): http::write: " << ec.message()
                          << std::endl;
            }
            ...
            if (ec) {
                std::cerr << "Impl::send(): http::read: " << ec.message()
                          << std::endl;
            }
...
Cache::val_type Cache::get(key_type key) const {
    ...
    if (response.result() == http::status::not_found) return {nullptr, 0};
